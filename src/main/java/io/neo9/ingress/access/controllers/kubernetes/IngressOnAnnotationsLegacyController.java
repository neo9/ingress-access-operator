...
@Component
@Slf4j
public class IngressOnAnnotationsLegacyController extends ReconnectableSingleWatcher<Ingress, IngressList> {

	public IngressOnAnnotationsLegacyController(KubernetesClient kubernetesClient, VisitorGroupIngressReconciler visitorGroupIngressReconciler, AdditionalWatchersConfig additionalWatchersConfig) {
		super(
				/* activation condition */
				additionalWatchersConfig.watchIngressAnnotations().isEnabled(),
				/* unique name */
				"ingress-onAnnotationsLegacy",
				/* watch what */
				kubernetesClient.network().v1().ingresses()
						.inAnyNamespace()
						.withoutLabel(LEGACY_MUTABLE_LABEL_KEY, MUTABLE_LABEL_VALUE),
				/* post watch filter */
				ingress -> MUTABLE_LABEL_VALUE.equalsIgnoreCase(getAnnotationValue(LEGACY_MUTABLE_LABEL_KEY, ingress)),
				/* on event */
				(action, ingress) -> {
